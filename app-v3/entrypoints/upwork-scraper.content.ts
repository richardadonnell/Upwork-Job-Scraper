...
function extractPublishedOnByUid(): Map<string, number> {
	...
	for (const script of scripts) {
		...
		if (!text || !text.includes("publishedOn") || !text.includes("uid"))
			continue;
...
export default defineContentScript({
	...
	main(): ScrapeResult {
		initSentryContext("content");

		const jobTiles = document.querySelectorAll<HTMLElement>(
		...
		jobTiles.forEach((article) => {
			try {
				...
				const title =
					...
					fallbackTitleEl?.textContent?.trim() ||
					"";
				...
				const parsedRelativePostedAtMs = parsePostedTextToMs(
					datePosted,
					scrapedAtMs,
				);
				...
				if (
					typeof absolutePostedAtMs === "number" &&
					Number.isFinite(absolutePostedAtMs)
				) {
					postedAtMs = absolutePostedAtMs;
					postedAtSource = "upwork_absolute";
				} else if (
					typeof parsedRelativePostedAtMs === "number" &&
					Number.isFinite(parsedRelativePostedAtMs)
				) {
				...
				const descEl = article.querySelector(
					".air3-line-clamp-wrapper.clamp p",
				);
				...
				const fixedBudgetEl = article.querySelector(
					'[data-test="is-fixed-price"] strong.rr-mask',
				);
				const hourlyBudgetEl = article.querySelector(
					'[data-test="is-hourly"] strong.rr-mask',
				);
				const budget =
					(fixedBudgetEl ?? hourlyBudgetEl)?.textContent?.trim() ?? "";
				...
				captureContextException("content", err, {
					operation: "parse_job_tile",
				});
			}
		});
