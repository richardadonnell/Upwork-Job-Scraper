...
export function JobHistoryTab() {
  ...
  const historyLabel = jobs.length === 0 ? '> no jobs scraped yet' : `> ${jobs.length} ${jobWord} in history`;
        ...
        >
          ...
          {(job.jobType || job.budget || job.experienceLevel) && (
            <div style={{ marginTop: 8, display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {[job.jobType, job.budget, job.experienceLevel].filter(Boolean).map((val) => (
                <span key={val} style={{
                  padding: '2px 8px', background: 'var(--surface-raised)',
                  border: '1px solid var(--border)', borderRadius: 4,
                  fontSize: 11, color: 'var(--text-mid)', fontFamily: 'var(--mono)',
                }}>
                  {val}
                </span>
              ))}
          ...
          {job.description && (
            <p style={{
              margin: '10px 0 0', fontSize: 12, color: 'var(--text-mid)',
          ...
          <div style={{
            ...
            display: 'flex', gap: 14, fontFamily: 'var(--mono)',
          }}>
            {job.paymentVerified && (
